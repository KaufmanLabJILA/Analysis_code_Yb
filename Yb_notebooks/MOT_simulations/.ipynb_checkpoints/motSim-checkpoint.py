...
class motBeam:
	...
	def __init__(self, k, w0, r0, gamma, delta, s0, pol, l=3):
		self.k = k
...
class motBeamShell(motBeam):
	def Sat(self, r):
		...
		z = drm*np.cos(theta)
		rho = np.abs(drm*np.sin(theta))
		return self.s0 * ((self.l/2)**(-self.l/2)) * np.exp(self.l/2) * ((self.w(z)/self.w0)**2) * ((rho*np.sqrt(2)/self.w(z))**self.l) * np.exp( (-2*(rho**2))/((self.w(z))**2) ) #* (1-(rho**2)/((self.w(z))**2))
