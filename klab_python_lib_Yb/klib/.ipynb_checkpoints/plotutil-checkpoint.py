...
#             plt.show()
        ...
        if plotROIs:
            ...
            plt.xlabel(parsed_data.keyName, fontsize = 14)
            plt.title("File " + str(parsed_data.fnum))
            plt.legend(range(len(parsed_data.masks)))
            plt.show()
...
def plotLosses(parsed_data):
    if parsed_data.keySort.ndim==1:
        ...
        np.save(parsed_data.data_address+'losses/'+str(parsed_data.fnum),arr([ks, losses,losserrs]))
...
def plotTimeFill(parsed_data, plotROIs = False):
    ...
    roisums = parsed_data.roisums[0].reshape(parsed_data.nreps, parsed_data.npics//2, 2, len(parsed_data.masks))
...
def plotIndLosses(parsed_data, plot = True):
    ...
    for rois in range(parsed_data.masks.shape[0]):
        losses =[]
        losserrs = []
        infids = []
...
def plotROILosses(parsed_data, plotTransfer = True, ptnum = 0, masks = False, n0=0, n1=0, vlim = -1):
    ...
    if n0==0:
        if masks:
            n0 = np.sqrt(len(parsed_data.masks)).astype(int)
            n1 = np.sqrt(len(parsed_data.masks)).astype(int)

        else:
            n0 = np.sqrt(len(parsed_data.rois)).astype(int)
            n1 = np.sqrt(len(parsed_data.rois)).astype(int)
    for rois in range(n0*n1):
        infidelity, inf_err, lossfrac, loss_err = hist_stats_roi(parsed_data.roisums, i=ptnum, thresh = parsed_data.thresh, countsperphoton = parsed_data.countsperphoton, rois = rois, quiet = True, plots = False)
        losses.append(lossfrac)
        infids.append(infidelity)
        losserrs.append(loss_err)
#             maxcoord = ks[np.argmax(losses)]
    losses = np.reshape(arr(losses), (n0, n1))  
    losserrs = np.reshape(arr(losserrs), (n0, n1)) 
    ...
    if not plotTransfer:
        plt.imshow(losses, aspect ='auto',vmin = vn, vmax = vx)
        cbar = plt.colorbar()
        cbar.set_label('Loss %')
        plt.title("File " + str(parsed_data.fnum))
        plt.show()
        ...
        transfer = 100-losses
        plt.imshow(transfer, aspect ='auto', vmin = vn, vmax = vx)
...
def plotROIFills(parsed_data, ptnum = -1, masks = False, n0=0, n1=0):
    thresh = parsed_data.thresh*parsed_data.countsperphoton
    binarized = np.clip(parsed_data.roisums, thresh, thresh+1) - thresh
    if ptnum == -1:
        fill_rois = np.mean(binarized[:,:,0,:], axis = (0,1))
    else:
        fill_rois = np.mean(binarized[ptnum,:,0,:], axis = 0)
#     fill = np.clip(parsed_data.roisums, parsed_data.thresh, parsed_data.thresh+1) - parsed_data.thresh
#     fill_rois = np.mean(fill[:,:,0,:], axis = (0,1))
    ...
    fills = np.reshape(arr(fill_rois), (n0, n1))  

    plt.imshow(fills, aspect ='auto')
    cbar = plt.colorbar()
    cbar.set_label('Fill fraction')
    plt.title("File " + str(parsed_data.fnum))
    plt.show()
    return fills
...
def fitIndividualData(parsed_data):
    centers = []
    ks = arr(parsed_data.keySort)
    for rois in range(len(parsed_data.masks)):
        losses = []
        losserrs = []
        infids = []
...
#             p_guess = [np.max(losses)/2, np.max(losses), np.max(losses)/2, .1, .1, ks[np.argmax(losses)], 0.1, 0]
        ...
        guess = [.05, .1, .05, .1, .1, 80.1, .1, 0]
        popt, pcov = curve_fit(triplor, ks, -(arr(losses)-100)/100, p0 = guess, maxfev=100000, bounds = (0,[1,1,1,1,1,np.inf,1,1]))
        perr = np.sqrt(np.diag(pcov))
...
#         plt.plot(ks, losses, 'k-')
        plt.plot(ks, -(arr(losses)-100)/100, 'k.')
        plt.plot(xs, triplor(xs, *popt), 'r-')
        plt.xlabel(parsed_data.keyName)
        plt.ylabel('losses')
...
#     nbar = []
#     nbar_err = []
#     for rois in range(len(parsed_data.rois)):
#         losses =[]
#         losserrs = []
#         infids = []
#         for var in range(parsed_data.roisums.shape[0]):
...
def plotIndSpect(parsed_data, plot = True, transfer = True, amp=.7, y0fit = False, guess = []):
    cents = []
    cents_err = []
    wids =[]
    wids_err = []
...
#             maxcoord = ks[np.argmax(losses)]
        ...
        if transfer:
            losses = -(losses-100)/100
            losserrs = losserrs/100
...
#         plt.plot(xs, triplor(xs, *popt), 'r-')
        ...
        try:
            popt, perr = gausfit(ks, losses, y_offset=y0fit, negative=False, guess = guess)
            xs = np.linspace(np.min(ks), np.max(ks), 100)
            plt.plot(xs, gaussian(xs, *popt,0), 'r-')
            cents.append(popt[0])
            cents_err.append(perr[0])
            wids.append(popt[2])
            wids_err.append(perr[2])
        except RuntimeError:
            cents.append(0)
            cents_err.append(0)
            wids.append(0)
        ...
        plt.xlabel(parsed_data.keyName)
        plt.ylabel('losses')
        plt.axis([min(xs), max(xs), 0, 1.1*max(losses)])
        plt.title(rois)
        ...
        if plot:
            ...
            plt.close()
    return cents, cents_err, wids, wids_err
#     return cents, cents_err, df, df_err, nbar, nbar_err


def printInfo(popt, pcov):
    print("ratio:")
    ...
    print("splitting:")
    print(str(1000*popt[6])+' kHz +- '+str(1000*np.sqrt(pcov[6][6]))+' kHz')

    print('n_bar:')
    print(1/(np.abs(popt[0]/popt[2])-1))
    print('n_bar bound:')
    print(1/(np.abs(popt[0])/(np.abs(popt[2])+np.sqrt(pcov[2][2]))-1))

    print('[a0, a1, a2, sigma_carrier, sigma_side, f_carrier, df_sidebands, y0]')
    print(popt)

    print(popt[-3])
    print(np.sqrt(pcov[-3][-3]))
    print("bsb:" + str(popt[-3] - popt[-2]))