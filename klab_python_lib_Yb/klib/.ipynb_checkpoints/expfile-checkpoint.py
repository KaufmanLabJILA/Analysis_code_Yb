...
#             return 'No-Variation', arr([1])
    ...
    def __return_hdf5_obj(self,obj,prefix):
        di = {}
        for o in obj:
            if type(obj[o]) == h5._hl.dataset.Dataset:
                di[str(o)] = self.__str_ds(obj[o],prefix+'\t')
            else:
                raise TypeError('This Type Note Supported')
        return di
...
#         self.print_script(self.f['NIAWG']['Seq. 1 NIAWG-Script'])
    ...
    def return_variables(self):
        return self.__return_hdf5_obj(self.f['Master-Parameters']['Variables'],'')
